...
if  [ ! -n "$UMI_ENV" ] ;then
    output="anyong-zip-dist"
else
    output="${UMI_ENV}-anyong-zip-dist"
fi

# 删除之前打包好的项目

rm -rf ./*-zip-dist ./dist ./anyong*.zip
mkdir ./${output}
mkdir ./${output}/bin
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/anyong/'${UMI_ENV}'_anyong/g' ./${output}/package.json
  else 
    sed -i 's/anyong/'${UMI_ENV}'_anyong/g' ./${output}/package.json
  fi
fi
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/anyong/'${UMI_ENV}'_anyong/g' ./${output}/bin/stop.sh
  else 
    sed -i 's/anyong/'${UMI_ENV}'_anyong/g' ./${output}/bin/stop.sh
  fi
fi
